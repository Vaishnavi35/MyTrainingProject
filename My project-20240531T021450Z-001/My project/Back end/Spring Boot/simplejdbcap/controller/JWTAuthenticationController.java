...
import com.example.simplejdbcap.model.JWTRequest;
...
public class JWTAuthenticationController {
	...
	public ResponseEntity<?> createAuthenticationToken(@RequestBody HttpServletRequest request,JWTRequest authenticationRequest,User user,UsernamePasswordAuthenticationToken authentication) throws Exception {
		...
		if(!checkName) {
			json_response = "{\"status\":" + g.toJson("UNAUTHORIZED") + ",\"UserName\":"+ g.toJson(authenticationRequest.getUsername()) 
		...
		if(Password){
				...
				/*
				 * try{ session=request.getSession(); String un="vaishu123";
				 * session.setAttribute("username", un); }catch(Exception e) { }
				 *//*
					 ...
					 */
				json_response = "{\"status\":" + g.toJson("OK") + ",\"UserName\":"+ g.toJson(authenticationRequest.getUsername()) 
